...
이 글에서는 AI 영상 프롬프트가 무엇인지, 왜 중요한지, 어떻게 구조화해야 하는지, 16:9와 9:16에서 무엇이 달라지는지, 그리고 바로 복붙해서 쓸 수 있는 예시까지 정리합니다. 이미 구도와 움직임 중심 가이드를 따로 정리해 두었기 때문에, 읽으면서 [[ko/blog/ai-video/camera-techniques/shot-composition-camera-movement|화면 구도와 카메라 움직임 가이드]]와 함께 보면 이해가 더 빨라집니다.
...
넷째, 여러 컷 구조로 확장하기 쉽습니다. 실제로 강한 AI 영상은 한 컷짜리 예쁜 장면보다, 짧아도 `도입 -> 상승 -> 클라이맥스 -> 종료` 흐름이 있는 경우가 많습니다. 최근 정리한 [[ko/blog/ai-video/seedance/seedance-manhattan-wall-run-portal-prompt|Seedance 포털 액션 아카이브]]처럼 컷 단위로 구조를 만들면 훨씬 강한 결과가 나옵니다.
...
이 부분은 이미 별도 글에서 더 길게 다뤘기 때문에, 구체적인 샷 예시가 필요하면 [[ko/blog/ai-video/camera-techniques/shot-composition-camera-movement|구도와 카메라 움직임 가이드]]를 같이 참고하는 편이 좋습니다.
